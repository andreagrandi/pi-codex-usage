...
const WIERD_STATUS_CHANNEL = "notify:status";
const WIERD_STATUS_SOURCE = "codex-usage";
...
export default function codexUsage(pi: ExtensionAPI) {
	...
	const setUsageStatusline = (
		...
		emitCodexUsageChips(pi, report, options.model);
...
function emitCodexUsageChips(
	...
	const detail = formatCodexUsageStatusline(report, model);
	const parts: string[] = [];
	if (snapshot.primary) parts.push(`${formatRemainingPercent(snapshot.primary)} (5h)`);
	if (snapshot.secondary) parts.push(`${formatRemainingPercent(snapshot.secondary)} (7d)`);
	if (parts.length === 0) {
		emitCodexUsageErrorChip(pi);
		return;
	}
	emitCodexUsageChip(pi, "usage", parts.join(" / "), "📊", detail);
	emitCodexUsageChip(pi, "5h", "", "⏱", undefined, "cleared");
	emitCodexUsageChip(pi, "7d", "", "📅", undefined, "cleared");
}

function emitCodexUsageErrorChip(pi: ExtensionAPI): void {
	emitCodexUsageChip(pi, "usage", "usage error", "⚠️", "Codex usage fetch failed", "error");
}

function clearCodexUsageChips(pi: ExtensionAPI): void {
	emitCodexUsageChip(pi, "5h", "", "⏱", undefined, "cleared");
	emitCodexUsageChip(pi, "7d", "", "📅", undefined, "cleared");
	emitCodexUsageChip(pi, "usage", "", "⚠️", undefined, "cleared");
}

function emitCodexUsageChip(
	pi: ExtensionAPI,
	id: string,
	label: string,
	icon: string,
	detail?: string,
	state: "active" | "error" | "cleared" = "active",
): void {
	pi.events.emit(WIERD_STATUS_CHANNEL, {
		source: WIERD_STATUS_SOURCE,
		id,
		state,
		label,
		icon,
